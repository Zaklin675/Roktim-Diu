					...
					<div class="card-body">
						<form method="POST" action="{{route('donor.contact')}}" class="contact-donor-form">
							...
							<div class="form-group">
								<input type="date" name="date" value="{{old('date')}}" class="form--control form-control-md" placeholder="@lang('Enter Date For Blood')" maxlength="80" required="">
							</div>
							<div class="form-group">
								<input type="text" name="location" value="{{old('location')}}" class="form--control form-control-md" placeholder="@lang('Enter Location')" maxlength="255" required="">
							</div>
							...
							@else
							<a  href="{{route('user.login.form')}}" class="btn btn--base w-100">Login First</a>
							@endif
						</form>
					</div>
				</div>
